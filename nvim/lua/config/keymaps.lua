...
-- See nvim/FORMATTING_NOTES.md for why LSP/treesitter must be neutralized.
local reflow = require("config.reflow")

local function opfunc_map(force_restyle)
  return function()
    reflow._pending_restyle = force_restyle
...
map("n", "gq", opfunc_map(false), { expr = true, desc = "Reflow (mode-aware)" })
map("n", "gqq", function()
  reflow._pending_restyle = false
  vim.o.operatorfunc = "v:lua.require'config.reflow'.opfunc"
  return "g@_"
end, { expr = true, desc = "Reflow line (mode-aware)" })
map("n", "gQ", opfunc_map(true), { expr = true, desc = "Restyle (formatter)" })
map("n", "gQQ", function()
  reflow._pending_restyle = true
  vim.o.operatorfunc = "v:lua.require'config.reflow'.opfunc"
  return "g@_"
end, { expr = true, desc = "Restyle line (formatter)" })
map("x", "gq", visual_map(false), { desc = "Reflow selection (mode-aware)" })
map("x", "gQ", visual_map(true), { desc = "Restyle selection (formatter)" })

map("n", "j", "v:count == 0 ? 'gj' : 'j'", { desc = "Down a display line", expr = true, silent = true })
map("n", "k", "v:count == 0 ? 'gk' : 'k'", { desc = "Up a display line", expr = true, silent = true })
map("n", "0", "g0", { desc = "Start of display line", silent = true })
map("n", "^", "g^", { desc = "First non-blank of display line", silent = true })
map("n", "$", "g$", { desc = "End of display line", silent = true })
...
map({ "n", "t" }, "<C-h>", "<Cmd>wincmd h<CR>", { desc = "Window left", silent = true })
map({ "n", "t" }, "<C-j>", "<Cmd>wincmd j<CR>", { desc = "Window down", silent = true })
map({ "n", "t" }, "<C-k>", "<Cmd>wincmd k<CR>", { desc = "Window up", silent = true })
map({ "n", "t" }, "<C-l>", "<Cmd>wincmd l<CR>", { desc = "Window right", silent = true })
map("t", "<Esc>", [[<C-\><C-n>]], { desc = "Terminal normal mode", silent = true })
map("t", "<C-w>", [[<C-\><C-n><C-w>]], { desc = "Window command", silent = true })
...
map("n", "<leader>cm", function()
  require("config.deps").check_current_buffer(0, { force = true })
end, { desc = "Missing dependencies (this buffer)", silent = true })
map("n", "<leader>cM", "<Cmd>checkhealth config<CR>", { desc = "All dependencies (:checkhealth config)", silent = true })
...
map("n", "<leader>tl", function()
  toggle_window_option("list")
end, { desc = "List characters (alias of SPC tvt)", silent = true })
